...
function http_get(url)
    ...
    headers = [
        s[1] => s[2] for s in split.(split(headers_str, "\r\n"), r": *") if length(s) == 2
    ]
    return headers, body
end
...
function post_error(path::AbstractString, ex::Exception)
    ...
    try
        ...
        http_post(
            url, headers, """{"errorType": "$(typeof(ex))", "errorMessage": "$(ex)"}"""
        )
...
while true
    ...
    try
        ...
        request_id_idx = findfirst(
            x -> x[1] == "Lambda-Runtime-Aws-Request-Id", request_headers
        )
